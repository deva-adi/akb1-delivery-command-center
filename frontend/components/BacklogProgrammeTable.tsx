...
/**
 * Backlog programme table with drill interactivity (M10-6).
 *
 * Click a row -> sets ?p=CODE
 *
 * activeProgramme comes from backlog-health/page.tsx (server searchParams).
 * useRouter is used for navigation; useSearchParams is NOT used.
 *
...
export function BacklogProgrammeTable({ states, activeProgramme }: Props) {
  const router = useRouter();

  function handleRowClick(code: string) {
    const params = new URLSearchParams();
    params.set("p", code);
    router.push(`/home/backlog-health?${params.toString()}`, { scroll: false });
  }
            ...
            {states.map((row, idx) => {
              ...
              return (
                <tr
                  ...
                  data-testid={`backlog-row-${row.code}`}
                >
