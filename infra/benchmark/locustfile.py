"""
AKB1 Delivery Command Center -- Locust benchmark (M8-5, updated M10-8).
...
Endpoint mix (approximates real usage pattern from Architecture doc 05):
  ...
  -- M10-8 drill endpoint additions --
  GET /api/v1/programmes                         -- 1%  filter bar load (all tabs)
  GET /api/v1/programmes/{code}                  -- 1%  programme detail (drill Level 1)
  GET /api/v1/people?programme={code}&band={b}   -- 1%  workforce/capability drill
  GET /api/v1/audit/entry/{entry_id}             -- 1%  audit trail Level 2
...
class AKB1User(HttpUser):
    ...
    # M10-8 drill endpoint additions -- weight 1 each (low frequency, realistic drill pattern)

    @task(1)
    def programmes_list(self):
        self.client.get("/api/v1/programmes", headers=_AUTH,
                        name="/api/v1/programmes")

    @task(1)
    def programme_detail(self):
        self.client.get(f"/api/v1/programmes/{_code()}", headers=_AUTH,
                        name="/api/v1/programmes/{code}")

    @task(1)
    def people_drill(self):
        bands = ["B1", "B2", "B3", "B4", "B5"]
        band = random.choice(bands)
        self.client.get(
            f"/api/v1/people?programme={_code()}&band={band}",
            headers=_AUTH,
            name="/api/v1/people?programme={code}&band={band}",
        )

    @task(1)
    def audit_entry_detail(self):
        # Uses a known seeded entry UUID from the deterministic seed.
        # Seed entry_id at index 0: generated by generator.py fixed-seed run.
        # Update this UUID if the seed is regenerated.
        audit_entry_id = "00000000-0000-0000-0000-000000000001"
        self.client.get(
            f"/api/v1/audit/entry/{audit_entry_id}",
            headers=_AUTH,
            name="/api/v1/audit/entry/{entry_id}",
        )
